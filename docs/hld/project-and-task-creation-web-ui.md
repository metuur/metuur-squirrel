...
**After this ships:**
- Users can create a new project directly from the SPA with full field support (tag, type, deadline, stakeholders, description, and optionally an initial task).
...
- SPA users can create a project via a form that exposes all current backend fields (`tag`, `type`, `deadline`, `stakeholders`, `description`, `first_intent_tag`, `first_intent_title`).
...
- Inline validation catches malformed `tag` and invalid `type` values before the network call.
...
4. Submitting a project with a `tag` that violates `^[A-Z][A-Z0-9]*(-[A-Z0-9]+)*$` or an invalid `type` is blocked client-side with a clear error message.
