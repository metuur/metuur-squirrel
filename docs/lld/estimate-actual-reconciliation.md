...
- **Single variance injection point: `_build_pick` (`focus_picker.py:170`).** That is the only
  object carrying `time_invested_minutes` to _both_ UIs (`manual_focus` in `/api/home`,
...
- `FocusWidget.tsx` (which already renders `time_invested_minutes`) gains:
  ...
  - a variance line when `has_variance`: `estimated time 45m · est 2h15m · actual 2h10m · 1.0×` using
...
- Frontmatter writes are atomic **per `write_frontmatter` call** (temp + `os.replace`,
  ...
  estimate-write and checkout `_update_time_invested` touch _disjoint_ keys, so the merge is safe;
