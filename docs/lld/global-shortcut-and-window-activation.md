...
[tauri-plugin-global-shortcut]
       │ Cmd+Shift+S pressed (system-wide)
       ▼
  lib.rs (setup)
  shortcut handler
       │ calls
...
| `src/lib.rs` setup block (line 38–50) | Register `tauri_plugin_global_shortcut` plugin with `Cmd+Shift+S` handler calling `tray::show_main_window` |
...
`"CommandOrControl+Shift+S"` — resolves to `Cmd+Shift+S` on macOS. The `matches()` check in the handler uses `Modifiers::SUPER | Modifiers::SHIFT` + `Code::KeyS` on macOS.
...
- **`CommandOrControl+Shift+S` over `Ctrl+Shift+S`** — avoids "Save As" collision on macOS; macOS users expect Cmd-based app-level shortcuts.
