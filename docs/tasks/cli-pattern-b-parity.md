...
- [ ] 2.1 Create `apps/cli/lib/focus_cli.py` with `get`, `set`, `clear` subcommands (est: ~30m)
...
- [ ] 3.1 Add `checkin` and `checkout` subcommands to `focus_cli.py` (deps: 2.1, est: ~25m)
...
- [ ] 4.1 Add `history` subcommand to `focus_cli.py` (deps: 2.1, est: ~15m)
...
- [ ] 1.1 Rewrite `agent-pack/commands/sq-focus.md` to use `focus_cli.py` (deps: 2.1, 3.1, 4.1, est: ~20m)
...
- [ ] 5.1 Add Step 5.5 to `agent-pack/skills/session-start/SKILL.md` (deps: 3.1, est: ~10m)
...
- [ ] 6.1 Add Step 12.5 to `agent-pack/skills/session-end/SKILL.md` (deps: 3.1, est: ~10m)
...
- [ ] 8.1 Add `__main__` entry point with `snooze` subcommand to `apps/cli/lib/reminder_writer.py` (est: ~20m)
...
- [ ] 7.1 Create `agent-pack/commands/sq-reminders.md` with list and snooze commands (deps: 8.1, est: ~15m)
...
- [ ] 9.1 Add `first_intent_filename` param to `new_project_writer.py` (est: ~20m)
...
- [ ] 9.2 Update `agent-pack/commands/sq-new-project.md` to expose `--first-intent-filename` (deps: 9.1, est: ~10m)
