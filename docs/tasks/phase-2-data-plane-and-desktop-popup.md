...
- Estimates are wall-clock for one engineer in focus. ADHD buffer not pre-applied.
...
- [ ] 1.1 Add Phase 2 React deps to `apps/desktop/package.json` (deps: none, est: ~10m)
...
- [ ] 1.2 Scaffold `apps/desktop/src/api/client.ts` with hard-coded backend origin and typed methods (deps: 1.1, est: ~30m)
...
- [ ] 1.3 Implement `useBackend()` hook with 10s polling (deps: 1.2, est: ~30m)
...
- [ ] 1.4 Implement `BackendStatusBanner` component (deps: 1.3, est: ~15m)
...
- [ ] 2.1 `FocusWidget` reads `/api/home.focus` (deps: 1.2, 1.4, est: ~25m)
...
- [ ] 2.2 `DeadlinesWidget` reads `/api/home.pressing[]` (deps: 2.1, est: ~25m)
...
- [ ] 2.3 `ParakeetWidget` reads `/api/parakeet` (deps: 1.4, est: ~15m)
...
- [ ] 2.4 Refactor 2.1 + 2.2 to share `useHome()` (deps: 2.1, 2.2, est: ~15m)
...
- [ ] 3.1 Build `CaptureButton` + `CaptureModal` (deps: 1.2, 1.4, est: ~50m)
...
- [ ] 3.2 Wire offline-disable on CaptureButton (deps: 3.1, est: ~10m)
...
- [ ] 3.3 Implement minimal toast primitive (deps: none, est: ~25m, mutex: ui-primitives)
...
- [ ] 4.1 Add "Open Web UI" item to tray menu between "Open Squirrel" and "Background Watcher" (deps: 0.5, est: ~20m)
...
- [ ] 4.2 Add "Open Web UI" button to popup body (deps: 2.4, est: ~15m)
...
- [ ] 5.1 Set CSP in `apps/desktop/src-tauri/tauri.conf.json` (deps: 0.5, est: ~20m)
...
- [ ] 5.2 Audit popup for external resource references (deps: 5.1, est: ~15m, mutex: ui-primitives)
...
- [ ] 6.1 Mount the popup in `apps/desktop/src/App.tsx` (deps: 1.4, 2.1, 2.2, 2.3, 3.1, 4.2, est: ~20m)
...
- [ ] 6.2 Add `dev` Make targets for the two-process workflow (deps: none, est: ~10m)
...
| 5 | 5.1–5.2 | ~35m |
| 6 | 6.1–6.3 | ~1h |
| **Total** | **16 stories** | **~6h20m** (≈ 1.5 focused days with ADHD ×2 buffer) |
