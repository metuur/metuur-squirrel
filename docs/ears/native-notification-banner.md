...
| R-1.6 | IF the emitter is `terminal-notifier`, THE invocation SHALL include `-group org.squirrel.reminders`, `-sender com.metuur.squirrel`, `-title`, `-subtitle`, `-message`, `-open <deep-link-url>`, and the configured `-sound`. THE `<deep-link-url>` SHALL be composed per R-1.9. Sound selection is owned by `notification-sound-selection` (default `Submarine` per R-1.5, omitted when `Silent`); `-sender` is required per `notification-icon-branding` R-2.1 and is orthogonal to sound. (Updated: `-sender` now required — the `R-1.10` deferral is delivered by `docs/ears/notification-icon-branding.md`.) |
| R-1.10 | **Delivered / superseded** by `docs/ears/notification-icon-branding.md`. Squirrel-icon branding via `-sender com.metuur.squirrel` is honored once `com.metuur.squirrel` has emitted ≥1 notification via `UNUserNotificationCenter`; the branding spec satisfies that precondition with a one-shot first-launch Tauri bootstrap (branding Unit 1), restores `-sender` to R-1.6 above (branding Unit 2 / R-2.1), and documents the System Settings → Notifications → Squirrel grant (branding R-5.1). The `osascript` and `display dialog` fallbacks remain unbranded by macOS limitation. |
| R-1.9 | WHEN composing the deep-link URL for an item, THE SYSTEM SHALL use `squirrel://projects/<project>/<id>` IF `project` is non-empty AND `id != project`; ELSE THE SYSTEM SHALL use `squirrel://projects/<project>` IF `project` is non-empty; ELSE THE SYSTEM SHALL use `squirrel://projects/<id>` (legacy fallback when an item lacks a `project` field). |
